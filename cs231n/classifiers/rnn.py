...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # forward (2)
        captions_in_vec, w2v_cache = word_embedding_forward(captions_in, W_embed)
        ...
        # forward (4)
        predicted_caption, vocab_cache = temporal_affine_forward(rnn_hidden, W_vocab, b_vocab)

        # forward (5)
        loss, grad_loss = temporal_softmax_loss(predicted_caption, captions_out, mask, verbose=False)
        ...
        # backward (1)
        gred_features, grad_w_proj, grad_b_proj = affine_backward(dh0, affine_cache)

        # save all gradients
        grads = {k:v for k, v in zip(['W_embed', 'W_proj', 'b_proj', 'Wx', 'Wh', 'b', 'W_vocab', 'b_vocab'],
                                     [grad_w_embed, grad_w_proj, grad_b_proj, dWx, dWh, db,
                                      grad_w_vocab, grad_b_vocab])}
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(1, max_length):
            ...
            # (4)
            new_word_ids = scores.argmax(axis=1)
